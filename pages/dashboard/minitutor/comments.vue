<template>
  <div class="p-3">
    <div v-for="comment in comments" :key="comment.id" class="flex p-3 mb-3 bg-white rounded shadow">
      <div class="w-14">
      ...
      <div class="flex-1 pl-3">
        ...
        <nuxt-link :to="`/${comment.post.type}s/${comment.post.slug}`" class="hover:bg-primary-600 bg-primary-100 text-primary-600 inline-flex justify-center items-center px-3 h-7 text-xs font-semibold leading-none rounded hover:text-white">Lihat Postingan</nuxt-link>
      </div>
    </div>

    <block-blank v-if="!comments.length" text="Belum ada Komentar." />
  </div>
</template>
