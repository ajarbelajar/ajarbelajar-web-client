      ...
      <div class="video-uploader-wrapper">
        <div class="upload-message">
          {{
            loading
              ? uploadMessage
              : error || 'Klik atau seret video anda disini.'
          }}
        </div>
        <input
          ref="input"
...
export default {
  ...
  methods: {
    ...
    async handleChange(ev) {
      this.loading = true
      const url = `/minitutor/request-playlists/${this.playlist.id}/video`
      const config = {
      ...
      try {
        ...
        const playlist = await this.$axios.$post(url, data, config)
